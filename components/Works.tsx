    ...
    <section id="works" className="lc-sec-pad" style={{ background: "#1A1A1A", padding: "90px 32px 90px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative" }}>
        <div className="lc-reveal" style={{ textAlign: "center", maxWidth: 680, margin: "0 auto 56px" }}>
          ...
          <p style={{ fontSize: 18, color: "#bbb", lineHeight: 1.7, margin: 0 }}>每個專案都是一次完整的合作，作品案例持續更新中。</p>
        </div>
        <div className="lc-grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 26 }}>
          {works.map((w) => {
            ...
            const inner = (
              <>
                <div style={{ height: 200, overflow: "hidden", position: "relative" }}>
                  ...
                  <span style={{ position: "absolute", top: 14, left: 14, background: "#1A1A1A", color: "#FFCE00", fontWeight: 700, fontSize: 12, padding: "5px 12px", borderRadius: 999, zIndex: 1 }}>{w.tag}</span>
                  {linked && (
                    <span style={{ position: "absolute", top: 14, right: 14, background: "#FFCE00", color: "#1A1A1A", fontWeight: 800, fontSize: 12, padding: "5px 11px", borderRadius: 999, zIndex: 1 }}>查看網站 ↗</span>
                  )}
                </div>
            ...
            return linked ? (
              <a key={w.title} href={w.url} target="_blank" rel="noopener noreferrer" className="lc-work lc-reveal" style={cardStyle}>
                {inner}
              </a>
            ) : (
              <div key={w.title} className="lc-work lc-reveal" style={cardStyle}>
                {inner}
              </div>
            );
          })}
